...
(function() {
	...
	guitarapp.controller('searchController', function($scope, $http, $location) {
		
		$scope.getAllGuitars = function() {
			$http.get("/guitarshop/webapi/guitars")
			.then(function(response) {
				$scope.guitars = response.data;
				console.log('number of guitars: ' + $scope.guitars.length);
			}, function(response) {
